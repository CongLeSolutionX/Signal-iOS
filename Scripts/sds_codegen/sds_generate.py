...
def update_objc_snippet(file_path, snippet):
    ...
    snippet = (
        f'// This snippet is generated by {sds_common.pretty_module_path(__file__)}. Do not manually edit it, instead run `sds_codegen.sh`.'
        + '\n\n'
        + snippet
    )
...
class ParsedClass:
    ...
    def should_generate_extensions(self):
        ...
        if self.name in ('OWSDatabaseMigration', 'YDBDatabaseMigration', 'OWSResaveCollectionDBMigration', ):
            return False
        return self.super_class_name not in (
    ...
    def sorted_record_properties(self):
        ...
        next_property_order = 1 + max(all_property_orders, default=0)
...
class TypeInfo:
    ...
    def deserialize_record_invocation(self, property, value_name, is_optional, did_force_optional):
        ...
        value_expr = f'record.{property.column_name()}'
        ...
        if value_expr == 'record.id':
            value_expr = f'{initializer_param_type}(recordId)'
        elif is_optional:
            if deserialization_optional is not None:
                value_expr = f'SDSDeserialization.{deserialization_optional}({value_expr}, name: "{value_name}"{deserialization_conversion})'
        elif did_force_optional:
            if deserialization_not_optional is not None:
                value_expr = f'try SDSDeserialization.{deserialization_not_optional}({value_expr}, name: "{value_name}")'
        if value_name == 'conversationColorName':
            value_statement = f'let {value_name}: {"ConversationColorName"} = ConversationColorName(rawValue: {value_expr})'
        elif value_name == 'mentionNotificationMode':
            value_statement = f'let {value_name}: {"TSThreadMentionNotificationMode"} = TSThreadMentionNotificationMode(rawValue: {value_expr}) ?? .default'
        elif value_name == 'storyViewMode':
            value_statement = f'let {value_name}: {"TSThreadStoryViewMode"} = TSThreadStoryViewMode(rawValue: {value_expr}) ?? .default'
        elif self.is_codable:
            value_statement = f'let {value_name}: {initializer_param_type} = {value_expr}'
        elif self.should_use_blob:
            blob_name = f'{str(value_name)}Serialized'
            if is_optional or did_force_optional:
                serialized_statement = f'let {blob_name}: Data? = {value_expr}'
            else:
                serialized_statement = f'let {blob_name}: Data = {value_expr}'
            if is_optional:
                value_statement = f'let {value_name}: {self._swift_type}? = try SDSDeserialization.optionalUnarchive({blob_name}, name: "{value_name}")'
            else:
                value_statement = f'let {value_name}: {self._swift_type} = try SDSDeserialization.unarchive({blob_name}, name: "{value_name}")'
    ...
    def serialize_record_invocation(self, property, value_name, is_optional, did_force_optional):
        ...
        if property.field_override_serialize_record_invocation() is not None:
            ...
            if is_optional or did_force_optional:
                return f'optionalArchive({value_expr})'
            else:
                return f'requiredArchive({value_expr})'
        elif self._objc_type == 'NSDate *':
            if is_optional or did_force_optional:
                return f'archiveOptionalDate({value_expr})'
            else:
                return f'archiveDate({value_expr})'
        ...
        # elif self.is_numeric():
            ...
            if is_optional or did_force_optional:
                return f'archiveOptionalNSNumber({value_expr}, conversion: {serialization_conversion})'
            else:
                return f'archiveNSNumber({value_expr}, conversion: {serialization_conversion})'
...
class ParsedProperty:
    ...
    def convert_objc_class_to_swift(self, objc_type, unpack_nsnumber=True):
        ...
        if array_match is not None:
            split = array_match[2]
            return f'[{self.convert_objc_class_to_swift(split, unpack_nsnumber=False)}]'

        dict_match = re.search(r'^NS(Mutable)?Dictionary<(.+),(.+)> \*$', objc_type)
        if dict_match is not None:
            split1 = dict_match[2].strip()
            split2 = dict_match[3].strip()
            return f'[{self.convert_objc_class_to_swift(split1, unpack_nsnumber=False)}: {self.convert_objc_class_to_swift(split2, unpack_nsnumber=False)}]'
    ...
    def deep_copy_record_invocation(self, value_name, did_force_optional):
        ...
        if objc_type in simple_type_map:
            ...
            return [
                f'let {value_name}: {initializer_param_type} = modelToCopy.{model_accessor}'
            ]
        ...
        return (
            [
                ...
                f'let {value_name}: {initializer_param_type}',
                'if let %sForCopy = modelToCopy.%s {'
                % (
                    value_name,
                    model_accessor,
                ),
                f'   {value_name} = try DeepCopies.deepCopy({value_name}ForCopy)',
                '} else {',
                f'   {value_name} = nil',
                '}',
            ]
            if is_optional
                ...
                f'let {value_name}: {initializer_param_type} = try DeepCopies.deepCopy(modelToCopy.{model_accessor})',
            ]
        )
